...
# B06: Updated column names and English translations for PastEx and PresentEx
# These are ordered to match the English questions list from the data dictionary for 1.1 and 2.1
PASTEX_COLUMN_NAMES = [
...
if 1: # For each survey question convert answers to numbers, calculate medians
    ...
    # Interleave PastEx and PresentEx questions for output
    max_len = max(len(PASTEX_COLUMN_NAMES), len(PRESENTEX_COLUMN_NAMES))
    ...
    for i in range(max_len):
        ...
        # Process PresentEx question if it exists
        if i < len(PRESENTEX_COLUMN_NAMES):
